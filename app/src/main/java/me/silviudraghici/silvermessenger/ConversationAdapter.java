...
/**
 * Created by silvi on 2016-09-27.
 */

public class ConversationAdapter
        extends RecyclerViewCursorAdapter<ConversationAdapter.MessageViewHolder>{

    public static final String IMAGE_NUMBER_KEY = "image_num_key";
    public static final String CONVERSATION_NUMBER_KEY = "con_num_key";
    ...
    private PictureDirectory pictureList;
    ...
    public ConversationAdapter(Context context, Cursor cursor,
                               View.OnLongClickListener listener, String conNum,
                               int originator) {
        super(cursor);
        ...
        pictureList = PictureDirectory.getInstance(context, conNum);
    ...
    public MessageViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        MessageViewHolder holder
                = new ConversationAdapter.MessageViewHolder(view);
        ...
        holder.itemView.setOnLongClickListener(deleteListener);
        view.setTag(holder);
        return holder;
    }
    ...
    private void inflateImage(MessageViewHolder holder, String message) {
        String key = message.replace("manif//", "");
        File pic = new File(context.getFilesDir(), pictureList.getPath(key));
        Uri pictureUri = Uri.fromFile(pic);

        holder.pictureView.setImageDrawable(null);
        holder.pictureView.setManifestNum(key);
    ...
    private void displayDay(Cursor cursor, MessageViewHolder holder, Date date) {
        if (cursor.isFirst()) {
    ...
    private void displayPicture(View v) {
        ...
        startImageDisplay.putExtra(CONVERSATION_NUMBER_KEY, conNum);
